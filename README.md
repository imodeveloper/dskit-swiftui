...
- [Screen construction](Content/Screen-Construction.md): how complete DSKit screens are assembled from containers, sections, rows, and cards.
...
Before changing DSKit, read the [contribution guide](Content/CONTRIBUTING.md), the [workflow guide](Content/docs/WORKFLOWS.md), and the generated component or screen page for the area you are changing. Generated docs are not edited by hand; update Swift source comments, `Testable_*` examples, snapshots, or the generator, then run `cd Scripts && ./documentation_generator.sh`.
