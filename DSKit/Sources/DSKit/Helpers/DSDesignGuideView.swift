...
public struct DSDesignGuideViewModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        switch orientation {
        case .horizontal:
            content.background(DSDesignGuideView(spacing: appearance.spacing.value(for: .space8), orientation: .horizontal, divider: divider))
        case .vertical:
            content.background(DSDesignGuideView(spacing: appearance.spacing.value(for: .space8), orientation: .horizontal, divider: divider))
        case .both:
            content
                .background(DSDesignGuideView(spacing: appearance.spacing.value(for: .space8), orientation: .vertical, divider: divider))
                .background(DSDesignGuideView(spacing: appearance.spacing.value(for: .space8), orientation: .horizontal, divider: divider))
        }
    }
}
