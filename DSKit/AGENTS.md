...
- `DSImageView` remote image placeholders:
  - While a remote image is loading, show only the shaped image background.
  - If the remote image load fails, show `photo.badge.exclamationmark`.
  - The failure icon size should scale with the rendered image bounds, but remain relatively smaller on large images; keep it readable without dominating the surface.
